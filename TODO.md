...
### Backend
- [ ] Bật và cấu hình nhà cung cấp `ollama-local` làm mặc định hoặc fallback ưu tiên trong `AI_DEFAULT_CONFIG`; đảm bảo thông tin kết nối, timeout và giới hạn kích thước yêu cầu tương thích với hạ tầng nội bộ.
- [ ] Bổ sung endpoint kiểm tra tình trạng dịch vụ (`/api/ai/providers/test`) hỗ trợ Ollama, bao gồm ghi nhận log lỗi chi tiết để dễ giám sát.
- [ ] Thiết lập cơ chế cache và retry riêng cho Ollama nhằm phòng khi dịch vụ tạm thời không phản hồi.
- [ ] Viết test tích hợp xác nhận `/api/ai/chat` gọi đúng provider Ollama khi được cấu hình.
