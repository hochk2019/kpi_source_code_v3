...
- [x] Thêm file `.gitattributes` ở thư mục gốc với cấu hình `* text=auto` và quy định `*.{js,jsx,json,md} text eol=lf` nhằm đồng bộ line ending giữa Windows/Linux (đã thêm file và chạy `git add --renormalize .`).
- [x] Cập nhật `eslint.config.js` sử dụng quy tắc `linebreak-style` thống nhất (ưu tiên "unix") thay vì phân nhánh theo hệ điều hành (đã đặt cố định `["error", "unix"]`).
- [x] Mở rộng `package.json` để các lệnh `format:check` và `format:write` bao phủ toàn bộ `src/**/*.js(x)` cùng thư mục cấu hình quan trọng (đã bổ sung glob docs/server và tham số `--end-of-line lf`).
- [x] Chạy `pnpm format:write` và rà soát diff để đảm bảo chỉ có thay đổi định dạng/trailing space (đã chạy với cấu hình mới, bao phủ toàn bộ mã nguồn).
- [x] Chạy `pnpm lint` xác nhận không còn cảnh báo về line ending hay format (đã chạy, lint chỉ còn cảnh báo kỹ thuật tồn đọng).
