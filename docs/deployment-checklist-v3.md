...
- [ ] Chạy `pnpm install --frozen-lockfile` để đảm bảo không thay đổi dependency (có thể dùng script `scripts/run-all-checks.ps1`).
- [ ] Chạy `pnpm lint`, `pnpm test`, `pnpm build` và ghi nhận kết quả (tự động nằm trong script `scripts/run-all-checks.ps1`).
- [ ] Chạy `pnpm test:screenshot` để kiểm tra favicon/logo và tạo ảnh chụp Playwright phục vụ báo cáo triển khai (cũng được script hỗ trợ, có tham số `-SkipScreenshots` khi cần).
