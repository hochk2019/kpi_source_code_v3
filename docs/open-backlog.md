...
- `cng-ro9.4` - active wave-3 code slice for declaration read/query extraction behind the existing `src/lib/store.js` facade
...
- Active/resume-safe child chain:
  ...
  - `cng-ro9.4` - store-decl-read-extraction (in progress; wave-3 declaration read/query split)
