...
- Active bead: `cng-ro9.5`
- Resume from: declaration read/query da duoc tach sang `src/lib/declReadStore.js`; tiep tuc chay impact cho `previewDeclRows` va `saveDeclRows`, sau do tach declaration save/import pipeline sang module moi ma khong dong vao lifecycle mutations.
- Pending verify:
  - `gitnexus_impact(target=previewDeclRows,direction=upstream)`
  - `gitnexus_impact(target=saveDeclRows,direction=upstream)`
  - `pnpm bd:check`
