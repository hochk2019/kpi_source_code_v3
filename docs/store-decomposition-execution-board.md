...
- Resume from: shared core helpers da duoc tach sang `src/lib/storeCoreHelpers.js` va `src/lib/declMutationHelpers.js`; tiep tuc scan caller low-risk con import `@/lib/store.js`, migrate theo dot nho, va track residual shim consumers truoc khi khoa shim.
- Pending verify:
