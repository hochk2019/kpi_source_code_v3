...
- Active bead: `cng-ro9.2`
- Resume from: chay impact cho `getRules`/`setRules`, tao module rules persistence moi, va wire facade trong `src/lib/store.js` ma khong doi business logic `src/lib/rules.js`.
- Pending verify:
  - `gitnexus_impact(target=getRules,direction=upstream)`
  - `gitnexus_impact(target=setRules,direction=upstream)`
  - `pnpm bd:check`
