...
- Active bead: `cng-ro9.4`
- Resume from: chay impact cho `getDeclRows` va `getData`, tach declaration read/query helpers sang module moi, va giu declaration save/mutation lanes nguyen ven.
- Pending verify:
  - `gitnexus_impact(target=getDeclRows,direction=upstream)`
  - `gitnexus_impact(target=getData,direction=upstream)` neu facade alias nam trong scope sua
  - `pnpm bd:check`
