...
- ✅ Chuẩn hoá quy trình backup SQL Server 2008 R2 (full hằng ngày, diff mỗi 6h) và kiểm tra khôi phục định kỳ trên môi trường dự phòng (xem thêm `docs/operations/ecus-backup-plan.md`). Hệ thống KPI tiếp tục sử dụng SQLite và chỉ đọc dữ liệu từ ECUS5VNACCS, do đó nhiệm vụ tập trung vào việc phối hợp cùng đội vận hành ECUS để đảm bảo lịch backup, kiểm tra khôi phục và cập nhật nhật ký vận hành sau mỗi lần thử nghiệm.
...
- ✅ Thêm lớp kiểm tra đầu vào khi import Excel (giới hạn 5 MB, tối đa 5.000 dòng, cảnh báo ngày sai định dạng) để ngăn chặn dữ liệu độc hại hoặc lỗi Unicode.
- ✅ Bổ sung tab “Lịch sử export” trên giao diện quản trị để tra cứu bảng `export_audit` theo khoảng thời gian, loại báo cáo và từ khóa, hỗ trợ truy vết nhanh.
