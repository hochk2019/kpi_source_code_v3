import { coLineCount } from '../../../domain/src/co.js';
import {
  normalizeDateInput,
  normalizeDeclarationNumber,
  normalizeStatusKey,
} from '../../../domain/src/declSearch.js';
...
export function readDeclarationRowsSnapshot(database, snapshotKey = ACTIVE_BUSINESS_SNAPSHOT_KEY) {
  return readJsonRowSnapshot(database, DECLARATION_ROW_DEFINITION, snapshotKey);
}

export function readDeclarationRowsSnapshotState(database, snapshotKey = ACTIVE_BUSINESS_SNAPSHOT_KEY) {
...
export function writeDeclarationRowsSnapshot(database, rows, options = {}) {
  writeJsonRowSnapshot(database, DECLARATION_ROW_DEFINITION, rows, options);
}

export function deleteDeclarationRowsSnapshot(database, snapshotKey = ACTIVE_BUSINESS_SNAPSHOT_KEY) {
  deleteJsonRowSnapshot(database, DECLARATION_ROW_DEFINITION, snapshotKey);
}

export function readMstAssignmentRowsSnapshot(database, snapshotKey = ACTIVE_BUSINESS_SNAPSHOT_KEY) {
  return readJsonRowSnapshot(database, MST_ASSIGNMENT_ROW_DEFINITION, snapshotKey);
}

export function writeMstAssignmentRowsSnapshot(database, rows, options = {}) {
  writeJsonRowSnapshot(database, MST_ASSIGNMENT_ROW_DEFINITION, rows, options);
}

export function deleteMstAssignmentRowsSnapshot(database, snapshotKey = ACTIVE_BUSINESS_SNAPSHOT_KEY) {
  deleteJsonRowSnapshot(database, MST_ASSIGNMENT_ROW_DEFINITION, snapshotKey);
}

export function readRuleCollectionSnapshot(database, snapshotKey = ACTIVE_BUSINESS_SNAPSHOT_KEY) {
  if (!database || typeof database.prepare !== 'function') {
    return null;
  }
...
export function deleteRuleCollectionSnapshot(database, snapshotKey = ACTIVE_BUSINESS_SNAPSHOT_KEY) {
  if (!database || typeof database.prepare !== 'function') {
    return;
  }
...
export function readAdjustmentRowsSnapshot(database, snapshotKey = ACTIVE_BUSINESS_SNAPSHOT_KEY) {
  return readJsonRowSnapshot(database, ADJUSTMENT_ROW_DEFINITION, snapshotKey);
}

export function writeAdjustmentRowsSnapshot(database, rows, options = {}) {
  writeJsonRowSnapshot(database, ADJUSTMENT_ROW_DEFINITION, rows, options);
}

export function deleteAdjustmentRowsSnapshot(database, snapshotKey = ACTIVE_BUSINESS_SNAPSHOT_KEY) {
