...
- Re-read and applied `planning-with-files`, `lint-and-validate`, and `verification-before-completion`.
- Created and claimed bead `cng-2ch` to extract the remaining `ReportViewer.jsx` preference/localStorage helpers and schedule/export controller actions.
- Added `src/components/reporting/useReportViewerPreferences.js` and moved report-viewer localStorage persistence, sanitize helpers, quick-range/filter initialization, column visibility, detail page-size state, and adjustment page-size sanitization behind the new hook.
- Added `src/components/reporting/useReportViewerActions.js` and moved schedule draft/edit/reset/toggle/save/delete behavior plus export permission validation/dispatch behind the new hook.
- Rewired `src/components/ReportViewer.jsx` to consume both hooks, corrected the moved reporting-client import path during verification, and tightened affected effect dependency arrays so the extracted seam stays lint-clean.
- Added `tests/useReportViewerPreferences.test.jsx` and `tests/useReportViewerActions.test.jsx` to lock the new controller seams directly, while keeping `tests/reportViewer.test.jsx` green.
- `ReportViewer.jsx` is now `735` lines instead of `958`, so the last major reporting hotspot is below the project soft cap.
- Tracker updates:
  - `bd.cmd close cng-2ch` -> closed successfully
  - `bd.cmd ready --json` -> `[]`
- Verification:
...
## Session: 2026-03-15 Dashboard Overview Seam

### Phase I: cng-tuw startup and seam confirmation

- **Status:** in_progress
- Actions taken:
  - Re-read and applied `planning-with-files`, `lint-and-validate`, and `verification-before-completion`.
  - Verified bead `cng-tuw` is active and aligned it with the notebook state.
  - Re-read the `ReportViewer.jsx` seam around the schedule shell, dashboard overview, and scope explorer boundary.
  - Confirmed the next extraction target is the inline block that still composes the summary cards, `TrendLineChart`, `TeamPieWidget`, `TopStaffWidget`, and `ReportingAdjustmentsPanel`.
- Files created/modified:
  - `task.md` (updated)
  - `task_plan.md` (updated)
  - `findings.md` (updated)
...
### Phase I completion: dashboard overview extraction

- **Status:** complete
- Actions taken:
  - Added `src/components/reporting/ReportingDashboardOverview.jsx` and moved the inline dashboard overview composition out of `ReportViewer.jsx`.
  - Rewired `src/components/ReportViewer.jsx` to delegate summary cards, trend, team pie, top-staff widget, and adjustments panel composition through the new module while keeping parent state ownership intact.
  - Added `tests/reportingDashboardOverview.test.jsx` to lock overview rendering plus callback forwarding for top-staff controls and adjustments page size.
  - Closed bead `cng-tuw` and re-checked `bd ready --json`.
- Files created/modified:
  - `src/components/reporting/ReportingDashboardOverview.jsx` (created)
  - `src/components/ReportViewer.jsx` (updated)
  - `tests/reportingDashboardOverview.test.jsx` (created)
  - `task.md` (updated)
  - `task_plan.md` (updated)
  - `findings.md` (updated)
  - `progress.md` (updated)
- Verification:
  ...
  - `bd.cmd show cng-tuw` -> `CLOSED`
  - `bd.cmd ready --json` -> `[]`

- Active epic: none (`cng-z7u` is closed)
- Active task target: none
- Next action: wait for the next residual bead or identify the next `ReportViewer.jsx`/repo-hygiene slice when new work is queued.

## Session: 2026-03-14 ReportViewer Adjustments Panel Follow-Up

- Confirmed the original residual-convergence epic `cng-z7u` is fully closed; continuing `F7` maintainability work now requires a fresh tracker item instead of reopening the old map.
- Created and claimed `cng-f2z` to extract the inline KPI adjustments/applied-points panel from `src/components/ReportViewer.jsx`.
- Re-checked the current hotspot in `ReportViewer.jsx`: the adjustments block still spans the applied table, page-size selector, prev/next pagination, totals-by-category, pending list, and rejected list, so it is the clearest remaining reporting seam.
- Confirmed `adjustmentPageSize` must remain parent-owned because it persists into report-viewer preferences, while the panel-specific derivation and render logic can move behind a dedicated reporting component.
- Added `src/components/reporting/ReportingAdjustmentsPanel.jsx` and rewired `ReportViewer.jsx` to delegate the full adjustments/applied-points surface there instead of keeping the panel inline.
- Added `tests/reportingAdjustmentsPanel.test.jsx` to lock applied/pending/rejected rendering plus page-size/pagination behavior directly at the extracted seam.
- Tracker updates:
  - `bd.cmd close cng-f2z` -> closed successfully
  - `bd.cmd ready --json` -> no ready residual bead remained after closure
- Verification:
  ...
  - `git diff --check -- src/components/ReportViewer.jsx src/components/reporting/ReportingAdjustmentsPanel.jsx tests/reportingAdjustmentsPanel.test.jsx` -> passed

## Session: 2026-03-14 ReportViewer Scope Explorer Extraction

- Continued the same `F7` cleanup path by opening `cng-vlt` for the next clear `ReportViewer.jsx` seam: the scope selector card that still bundled the staff/team toggle, chooser, column-visibility toggles, and the final branch handoff into the two section components.
- Added `src/components/reporting/ReportingScopeExplorerPanel.jsx` and rewired `ReportViewer.jsx` so that shell becomes a dedicated reporting module instead of staying inline in the parent.
- Added `tests/reportingScopeExplorerPanel.test.jsx` to lock scope switching, select forwarding, and column-toggle wiring directly on the new module while keeping `tests/reportingScopeSections.test.jsx` focused on the staff/team section contracts.
- `ReportViewer.jsx` is now `1405` lines instead of `1515`; the parent no longer owns the scope explorer presentation shell and the residual reporting hotspot is narrower than before.
- Tracker updates:
  - `bd.cmd create ...` -> created `cng-vlt`
  - `bd.cmd update cng-vlt --status in_progress` -> claimed successfully
  - `bd.cmd close cng-vlt` -> closed successfully
  - `bd.cmd ready --json` -> still empty after closure
- Verification:
...
## Session: 2026-03-14 DataImporter Container Hook Extraction

- Continued `cng-z7u.5` by moving the remaining top-level importer orchestration out of `src/components/DataImporter.jsx` and into `src/components/dataImporter/useDataImporterContainerProps.js`.
- Rewired `DataImporter.jsx` into a thin `DataImporterShell` wrapper, so the parent now just forwards the assembled container props instead of owning the orchestration layer inline.
- Added `tests/useDataImporterContainerProps.test.jsx` to lock the extracted seam directly across session-controller wiring, duplicate-workflow composition, results-surface composition, and the final prop-builder handoff into `createDataImporterContainerProps()`.
- Fixed follow-up verification issues found during the slice: corrected the moved hook's relative imports to `packages/domain`, tightened the test's `@/lib/store.js` mock into a partial-module mock so config exports stay intact, and normalized the touched files with repo formatting.
- `DataImporter.jsx` is now `7` lines and `useDataImporterContainerProps.js` is `748` lines, so the importer parent/orchestration hot path is now under the project soft module-size target.
- Tracker updates:
  - `bd.cmd close cng-z7u.5` -> closed successfully
  - `bd.cmd ready --json` -> only the epic remains open; no ready child task is left in the mapped `cng-z7u` backlog
  - `bd.cmd close cng-z7u` was attempted, but direct-mode tracker startup did not complete the close operation, so the epic currently remains open for follow-up triage
- Verification:
  - `pnpm exec vitest run tests/useDataImporterContainerProps.test.jsx tests/dataImporterContainerProps.test.js tests/dataImporterShellProps.test.js tests/dataImporter.preview.test.jsx --environment jsdom` -> passed (`15/15`)
  - `pnpm exec eslint src/components/DataImporter.jsx src/components/dataImporter/useDataImporterContainerProps.js tests/useDataImporterContainerProps.test.jsx` -> passed (`0` errors, `0` warnings)
  - `git diff --check -- src/components/DataImporter.jsx src/components/dataImporter/useDataImporterContainerProps.js tests/useDataImporterContainerProps.test.jsx` -> passed

## Session: 2026-03-14 Reporting Chart Harness Cleanup

- Continued `cng-z7u.6` by isolating `recharts` responsive sizing behind an explicit jsdom test harness instead of leaving `ReportViewer` tests dependent on zero-size layout behavior.
- Updated `vitest.setup.js` to mock `ResponsiveContainer` with stable numeric dimensions for jsdom while leaving the rest of the `recharts` surface untouched.
- Strengthened `tests/reportingOverviewWidgets.test.jsx` with a populated chart-path regression that asserts the old zero-size warning pattern does not reappear while still rendering real reporting chart branches.
- The reporting warning-noise slice is now complete; the next active residual work is `cng-z7u.5` (`DataImporter.jsx` decomposition), not reporting harness cleanup.
- Verification:
  - `pnpm exec vitest run tests/reportingScopeSections.test.jsx tests/useReportViewerReadModel.test.jsx tests/reportingClient.test.js tests/reportingDetailCards.test.jsx tests/reportingOverviewWidgets.test.jsx tests/reportingPanels.test.jsx tests/reportViewer.test.jsx --environment jsdom` -> passed (`29/29`) with no known `Recharts` width/height flood in `tests/reportViewer.test.jsx`
  - `pnpm exec eslint vitest.setup.js tests/reportingOverviewWidgets.test.jsx tests/reportingScopeSections.test.jsx tests/useReportViewerReadModel.test.jsx tests/reportingClient.test.js tests/reportingDetailCards.test.jsx tests/reportingPanels.test.jsx tests/reportViewer.test.jsx` -> passed (`0` errors, `0` warnings)
  - `git diff --check -- vitest.setup.js tests/reportingOverviewWidgets.test.jsx` -> passed

## Session: 2026-03-14 ReportViewer Section Composition Extraction

- Continued `cng-z7u.4` by moving the last staff/team section composition out of `src/components/ReportViewer.jsx`.
- Added `src/components/reporting/ReportingStaffSection.jsx` and `src/components/reporting/ReportingTeamSection.jsx`, then rewired `ReportViewer.jsx` to delegate the remaining summary/detail section rendering through those components instead of keeping both large render blocks inline.
- Added focused regression coverage in `tests/reportingScopeSections.test.jsx` for section guardrail states, summary-mode control wiring, all-scope detail rendering, and selected staff/team detail-card ownership.
- `ReportViewer.jsx` is now `1942` lines instead of `2585`; `cng-z7u.4` is closed and the next active reporting slice is `cng-z7u.6` chart-warning/jsdom harness cleanup.
- Tracker updates:
  - `bd.cmd close cng-z7u.4` -> closed successfully
  - `bd.cmd update cng-z7u.6 --status in_progress` -> next slice claimed
- Verification:
  - `pnpm exec vitest run tests/reportingScopeSections.test.jsx tests/useReportViewerReadModel.test.jsx tests/reportingClient.test.js tests/reportingDetailCards.test.jsx tests/reportingOverviewWidgets.test.jsx tests/reportingPanels.test.jsx tests/reportViewer.test.jsx --environment jsdom` -> passed (`28/28`), with the existing known `Recharts` width/height jsdom warnings still present in `tests/reportViewer.test.jsx`
  - `pnpm exec eslint src/components/ReportViewer.jsx src/components/reporting/reportingDetailUtils.js src/components/reporting/ReportingStaffSection.jsx src/components/reporting/ReportingTeamSection.jsx tests/reportingScopeSections.test.jsx tests/useReportViewerReadModel.test.jsx tests/reportingClient.test.js tests/reportingDetailCards.test.jsx tests/reportingOverviewWidgets.test.jsx tests/reportingPanels.test.jsx tests/reportViewer.test.jsx` -> passed (`0` errors, `0` warnings)
  - `git diff --check -- src/components/ReportViewer.jsx src/components/reporting/reportingDetailUtils.js src/components/reporting/ReportingStaffSection.jsx src/components/reporting/ReportingTeamSection.jsx tests/reportingScopeSections.test.jsx` -> passed

## Session: 2026-03-14 ReportViewer Read-Model Hook Extraction

- Continued `cng-z7u.4` by moving the remaining report-fetch/schedule-subscription/reload orchestration out of `src/components/ReportViewer.jsx`.
- Added `src/components/reporting/useReportViewerReadModel.js` so report fetch, baseline-rule comparison, schedule read-model subscription, and reload/version handling now sit behind one dedicated hook instead of staying embedded in the parent screen.
- Added focused coverage in `tests/useReportViewerReadModel.test.jsx` for the new hook, including baseline loading, schedule subscription updates, error fallback to empty read models, and explicit reload refresh behavior.
- Rewired `ReportViewer.jsx` to consume the new hook while preserving the existing `subscribeStorage()` refresh triggers for declarations/rules/teams and keeping the known `Recharts` jsdom warning noise unchanged.
- `ReportViewer.jsx` is now `2585` lines instead of `2713`; the remaining `cng-z7u.4` residue is the large staff/team section composition, not request/state orchestration anymore.
- Verification:
  - `pnpm exec vitest run tests/useReportViewerReadModel.test.jsx tests/reportingClient.test.js tests/reportingDetailCards.test.jsx tests/reportingOverviewWidgets.test.jsx tests/reportingPanels.test.jsx tests/reportViewer.test.jsx --environment jsdom` -> passed (`23/23`), with the existing known `Recharts` width/height jsdom warnings still present in `tests/reportViewer.test.jsx`
  - `pnpm exec eslint src/components/ReportViewer.jsx src/components/reporting/useReportViewerReadModel.js src/components/reporting/StaffDetailCard.jsx src/components/reporting/TeamDetailCard.jsx src/components/reporting/reportingDetailUtils.js tests/useReportViewerReadModel.test.jsx tests/reportingDetailCards.test.jsx tests/reportingOverviewWidgets.test.jsx tests/reportingPanels.test.jsx tests/reportViewer.test.jsx` -> passed (`0` errors, `0` warnings)

## Session: 2026-03-14 ReportViewer Detail Card Extraction

- Continued `cng-z7u.4` after the overview/table seam by externalizing the staff/team detail-card layer and the shared detail pagination/sort helpers out of `src/components/ReportViewer.jsx`.
- Added `src/components/reporting/StaffDetailCard.jsx`, `src/components/reporting/TeamDetailCard.jsx`, and `src/components/reporting/reportingDetailUtils.js`, then rewired `ReportViewer.jsx` to consume those seams instead of carrying the detail-card render trees inline.
- Added focused coverage in `tests/reportingDetailCards.test.jsx` for the new staff/team detail boundary, including detail-mode pagination and team-member sort behavior.
- Confirmed the starting worktree matched the notebook before code changes (`git status` clean on `ux-improvement-plan`), and the current dirty state is limited to the expected reporting extraction files plus the notebook updates from this slice.
- `ReportViewer.jsx` is now `2713` lines instead of `5798`; the remaining `cng-z7u.4` residue is request/state orchestration and the large staff/team section composition, not detail-card ownership anymore.
- Verification:
  - `pnpm exec vitest run tests/reportingClient.test.js tests/reportingDetailCards.test.jsx tests/reportingOverviewWidgets.test.jsx tests/reportingPanels.test.jsx tests/reportViewer.test.jsx --environment jsdom` -> passed (`20/20`), with the existing known `Recharts` width/height jsdom warnings still present in `tests/reportViewer.test.jsx`
  - `pnpm exec eslint src/components/ReportViewer.jsx src/components/reporting/StaffDetailCard.jsx src/components/reporting/TeamDetailCard.jsx src/components/reporting/reportingDetailUtils.js tests/reportingDetailCards.test.jsx tests/reportingOverviewWidgets.test.jsx tests/reportingPanels.test.jsx tests/reportViewer.test.jsx` -> passed (`0` errors, `0` warnings)
...
## 2026-03-15 Postgres Runtime Frontend Compatibility Bridge

- **Status:** completed
- Actions taken:
  - Brought up a local Postgres-backed dev runtime on `127.0.0.1:5433` with database `kpi_dev_ui`, then seeded canonical runtime tables from the existing dev SQLite snapshot so the `server-v4` stack could boot with real business data.
  - Confirmed the canonical reporting surface works against Postgres via `GET /api/v4/reporting/view`, then verified bootstrap auth with the default local admin account.
  - Added `server-v4/src/app/legacyCompatRoutes.ts` and mounted it from `server-v4/src/app/build-v4-app.ts` to bridge the current frontend's legacy `/api/auth/*`, `/api/health`, and `/api/storage/:key` expectations onto the active `server-v4` runtime without reopening the wider route rollout.
  - Mapped the legacy shared-storage bootstrap keys onto typed runtime readers/stores where available, and returned explicit safe defaults for legacy-only keys that do not yet have a canonical Postgres-backed equivalent.
  - Restarted the API on `http://127.0.0.1:5001` and the frontend on `http://127.0.0.1:5174/`, then re-verified end-to-end login/session/bootstrap behavior through the legacy bridge.
- Files created/modified:
  - `server-v4/src/app/legacyCompatRoutes.ts` (created)
  - `server-v4/src/app/build-v4-app.ts` (updated)
  - `tests/server-v4/legacyCompatRoutes.test.js` (created)
  - `server/reportingProjectionSqlite.d.ts` (updated)
  - `server/reportingReadModels.d.ts` (created)
- Verification:
  - `pnpm exec vitest run tests/server-v4/legacyCompatRoutes.test.js` -> passed (`2/2`)
  - `pnpm build:server-v4` -> passed
  - Runtime smoke check with cookie session against `http://127.0.0.1:5001` -> passed (`/api/auth/login`, `/api/auth/session`, `/api/storage/decl_rows_v1`, `/api/storage/kpi_rules_v2`)
- Outcome:
  - The current frontend can now authenticate and bootstrap against the Postgres-backed `server-v4` runtime instead of depending on the legacy SQLite-only dev API shape.
  - The compatibility bridge is intentionally narrow; core login/bootstrap/reporting is verified, while unrelated legacy endpoints remain out of scope until they are explicitly migrated.

## 2026-03-15 Bootstrap Bridge Follow-Through

- **Status:** completed
- Actions taken:
  - Audited the real frontend bootstrap path and confirmed `src/lib/storageClient.js` still initializes shared state through `GET /api/bootstrap`, while the first compatibility slice only covered `/api/auth/*` plus `/api/storage/:key`.
  - Added a legacy-compatible `GET /api/bootstrap` route to `server-v4/src/app/legacyCompatRoutes.ts` that returns `{ data: store }` with the same raw-string/null contract the frontend expects from the monolith bootstrap lifecycle.
  - Reused the existing typed runtime readers/stores to assemble only the shared bootstrap keys the current frontend actually consumes, instead of reopening a broad `kv_store` emulation surface.
  - Extended `tests/server-v4/legacyCompatRoutes.test.js` to lock both the authenticated bootstrap snapshot shape and the unauthenticated `401` behavior.
- Files created/modified:
  - `server-v4/src/app/legacyCompatRoutes.ts` (updated)
  - `tests/server-v4/legacyCompatRoutes.test.js` (updated)
- Verification:
  - `pnpm exec vitest run tests/server-v4/legacyCompatRoutes.test.js` -> passed (`2/2`)
  - `pnpm build:server-v4` -> passed
  - Live runtime verification on `http://127.0.0.1:5001` after restart -> passed (`/api/auth/login`, `/api/bootstrap`)
- Outcome:
