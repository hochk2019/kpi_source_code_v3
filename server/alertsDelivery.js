...
export function resolveSeverity(input) {
  ...
  return SEVERITY_MAP.get(normalized) || (SEVERITY_COLORS[normalized] ? normalized : DEFAULT_SEVERITY);
}
...
export function buildPlainText(payload = {}) {
  ...
  const heading = title ? `[${severity.toUpperCase()}] ${title}` : `[${severity.toUpperCase()}] Cảnh báo hệ thống KPI`;
  lines.push(heading);
  ...
  if (alerts.length > 0) {
    ...
    for (const alert of alerts) {
      ...
      const missing = Array.isArray(alert?.missing) && alert.missing.length > 0 ? ` thiếu ${alert.missing.join(', ')}` : '';
  ...
  if (meta) {
    const entries = Object.entries(meta).filter(([, value]) => value !== undefined && value !== null && `${value}`.trim());
    if (entries.length > 0) {
      lines.push('');
...
export function getEmailConfig(env = process.env) {
  ...
  for (const prefix of prefixes) {
    ...
    const secure = parseBoolean(pickFirstConfig(env, [`${prefix}_EMAIL_SECURE`, `${prefix}_SMTP_SECURE`]), port === 465);
    const user = pickFirstConfig(env, [`${prefix}_EMAIL_USER`, `${prefix}_SMTP_USER`]);
    const pass = pickFirstConfig(env, [`${prefix}_EMAIL_PASS`, `${prefix}_SMTP_PASS`, `${prefix}_EMAIL_PASSWORD`]);
...
export function getTeamsConfig(env = process.env) {
  ...
  for (const prefix of prefixes) {
    ...
    const mentions = parseRecipients(pickFirstConfig(env, [`${prefix}_TEAMS_MENTIONS`, `${prefix}_MENTIONS`]));
...
function convertPlainToHtml(text) {
  return text.split('\n').map((line) => line.trim() ? `<p>${escapeTeamsMarkdown(line)}</p>` : '<br>').join('');
}
