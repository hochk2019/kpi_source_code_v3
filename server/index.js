...
import {
  ...
  AI_CONFIG_KEY,
  AI_INSIGHTS_KEY,
  AI_INSIGHT_DEFAULT_CRON,
  AI_INSIGHT_FEEDBACK_COMMENT_LIMIT,
  AI_INSIGHT_MAX_ENTRIES,
  AI_SNAPSHOT_CACHE_KEY,
  AI_SNAPSHOT_CACHE_LIMIT,
  AI_SNAPSHOT_CACHE_TTL_MS,
  AI_SNAPSHOT_HISTORY_KEY,
  AI_SNAPSHOT_HISTORY_MAX_ENTRIES,
...
const backupDomain = createBackupDomain({
  ...
  getBackupDirectory,
  getBackupConfig,
  getDefaultBackupRetention: () => DB_BACKUP_RETENTION,
  getState: () => ({
    dbBackupJob,
    backupInProgress,
    restoreInProgress,
  ...
  formatNextRunHuman,
  isCronDisabled: () => process.env.KPI_DISABLE_CRON === '1',
});
...
const MST_ASSIGNMENT_STATUS = Object.freeze({
...
export async function performDatabaseBackup({
  ...
  return backupDomain.performDatabaseBackup({ dbFile, backupDir, retention, reason, actor, note });
}
...
async function listBackupFiles({ backupDir = getBackupDirectory(), limit = 50 } = {}) {
  ...
  return backupDomain.listBackupFiles({ backupDir, limit });
}
async function restoreDatabaseBackup({
  ...
  return backupDomain.restoreDatabaseBackup({ filename, backupDir, dbFile, actor, note });
}
...
function buildBackupSummary({ limit = 10 } = {}) {
  ...
  return backupDomain.buildBackupSummary({ limit });
}
function refreshDatabaseBackupSchedule() {

  return backupDomain.refreshDatabaseBackupSchedule();
}
...
const aiChatHistoryStore = createAiChatHistoryStore({
  getValue,
  upsertValue,
  deleteValue,
  aiChatHistoryPrefix: AI_CHAT_HISTORY_PREFIX,
...
function sanitizeMSTRow(rowInput) {
  ...
  const statusRaw = pickFirstValue(
    ...
    ['status', 'trang_thai', 'trangThai', 'state'],
...
function getMSTRows() {
  ...
  return getMSTRowsRaw().map(sanitizeMSTRow).filter(Boolean).sort(compareMSTRows);
...
function createMSTHistoryEntry({ mst, field, from, to, actor, timestamp, rowKey, type }) {
  ...
  return {
    ...
    actor: normalizeStr(actor) || 'system',
...
function appendMSTHistoryEntries(entries) {
  ...
  const existing = getJSONValue(MST_HISTORY_KEY, []);
  ...
  setJSONValue(MST_HISTORY_KEY, merged, { skipMstHistorySync: true });
  ...
  if (resolveMstHistorySqlConfig()) {

    scheduleMstHistorySqlSyncFromJson(JSON.stringify(merged));
...
function upsertMSTRows(rows, { actor = 'system', detail = '' } = {}) {
  ...
  setJSONValue('mst_rows_v2', sanitized);
...
function ensureMSTEntriesForDeclRows(declRows, { actor = 'system', dryRun = false } = {}) {
  ...
  const merged = getMSTRowsRaw().concat(additions);
...
async function runEcusSync({
  ...
  const mstSummary = ensureMSTEntriesForDeclRows(storedRows, { actor });
