...
function buildCompanyRows(rows, options = {}) {
  ...
  return aggregated.map((item, idx) => {
    const base = [idx + 1, item.cong_ty || "", item.mst || ""];
...
export async function generateStaffReport(payload = {}, options = {}) {
  ...
  currentRow = addDataRows(
    sheet,
    currentRow + 1,
    buildSummaryRows(staffData.stats, staffData.adjustmentSummary),
  );
  ...
  const detailData = detailRows.length ? detailRows : [detailHeaders.map(() => "")];
...
export async function generateTeamReport(payload = {}, options = {}) {
  ...
  const teamDetailData = teamDetailRows.length ? teamDetailRows : [teamDetailHeaders.map(() => "")];
...
export async function generateAllStaffReport(payload = {}, options = {}) {
  ...
  currentRow = addDataRows(
    sheet,
    currentRow + 1,
    buildSummaryRows(summaryStats, summaryStats.adjustmentTotals),
  );
  ...
  const staffHeaders = ["STT", "Nhân viên", "Tổ đội", "Tờ khai", "Điểm KPI", "Nhập", "Xuất"];
...
export async function generateAllTeamReport(payload = {}, options = {}) {
  ...
  const teamHeaders = ["STT", "Tổ đội", "Thành viên", "Tờ khai", "Điểm KPI", "Nhập", "Xuất"];
