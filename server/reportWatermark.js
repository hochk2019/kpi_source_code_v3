...
function normalizeFilters(value, depth = 3, breadth = 10) {
  ...
  if (Array.isArray(value)) {
    const output = value
      .slice(0, breadth)
      .map((item) => normalizeFilters(item, depth - 1, breadth));
...
export function computeWatermarkSignature(metadata = {}) {
  const issuedAt =
    metadata.issuedAt instanceof Date && !Number.isNaN(metadata.issuedAt)
      ? metadata.issuedAt
      : new Date();
...
export function applyWorkbookWatermark(workbook, metadata = {}) {
  ...
  const { normalized: normalizedFilters, summary: filterSummary } = buildFilterSummary(
    metadata.filters || {},
  );
  ...
  rows.push([
    "Ghi chú",
    "Không xoá sheet này. Khi điều tra, đối chiếu mã xác thực với nhật ký tải.",
  ]);
