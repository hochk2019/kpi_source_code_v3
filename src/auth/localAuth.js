...
function setAccountCache(accounts) {
  const normalized = Array.isArray(accounts)
    ? accounts.map(normalizeUserRecord).filter(Boolean)
    : [];
...
export function createAuthHeaders(baseHeaders) {
  const headers =
    baseHeaders instanceof Headers
      ...
      : new Headers(baseHeaders || undefined);
...
export async function updateAccount(usernameInput, patch, { actor = "system" } = {}) {
  ...
  return (
    response?.account ?? accounts.find((account) => account.username === usernameInput) ?? null
  );
}
...
export async function setAccountPassword(
  ...
  const response = await requestJson(
    `/api/auth/accounts/${encodeURIComponent(usernameInput)}/password`,
    {
      method: "POST",
