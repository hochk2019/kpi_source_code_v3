...
import { BACKUP_REASON_LABELS, translateBackupFailure } from "@/shared/backupMessages.js";
  ...
  const handleCronSubmit = useCallback(
    async (event) => {
      ...
      try {
        ...
        if (
          payload?.config &&
          Object.prototype.hasOwnProperty.call(payload.config, "retentionCopies")
        ) {
    ...
    <div className="space-y-6">
      <section className="rounded border border-[color:var(--ds-border-subtle)] bg-[color:var(--ds-surface-card)] p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="text-sm font-semibold uppercase text-[color:var(--ds-text-muted)]">
              Lịch sao lưu CSDL
            </div>
        ...
        {summaryError ? (
          ...
          <div className="mt-3 text-sm text-[color:var(--ds-text-muted)]">
            Đang tải thông tin sao lưu...
          </div>
        ) : summary ? (
          <div className="mt-4 space-y-4 text-sm text-[color:var(--ds-text-secondary)]">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <div className="text-xs uppercase text-[color:var(--ds-text-muted)]">
                  Biểu thức cron
                </div>
              ...
              <div>
                ...
                {schedule?.active ? (
                  <div className="space-y-1 text-sm text-[color:var(--ds-text-primary)]">
                    <div>
                      {schedule?.nextRunHuman ||
                        (schedule?.nextRun ? formatTime(schedule.nextRun) : "Không xác định")}
                    </div>
              ...
              <div>
                ...
                <div
                  className="truncate text-sm text-[color:var(--ds-text-primary)]"
            ...
            {canManageBackups && (
              <form
                className="space-y-3 rounded border border-[color:var(--ds-border-subtle)] bg-[color:var(--ds-surface-muted)] p-3"
                onSubmit={handleCronSubmit}
              >
                <div className="flex flex-col gap-3 md:flex-row md:flex-wrap md:items-end">
                  <div className="flex-1 space-y-1">
                    <label
                      className="text-xs font-medium text-[color:var(--ds-text-secondary)]"
                      htmlFor="backup-cron-input"
                    >
                  ...
                  <div className="flex w-full flex-col gap-1 md:w-44">
                    <label
                      className="text-xs font-medium text-[color:var(--ds-text-secondary)]"
                      htmlFor="backup-retention-input"
                    >
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <form onSubmit={handleRestoreSubmit} className="flex flex-col gap-2">
                    ...
                    <select
                      ...
                      disabled={
                        loadingBackups || restoring || runningBackup || backupFiles.length === 0
                      }
                    >
                      {loadingBackups ? (
                    ...
                    <button
                      ...
                      disabled={
                        restoring || runningBackup || loadingBackups || backupFiles.length === 0
                      }
                    >
                    ...
                    <p className="text-xs text-[color:var(--ds-text-muted)]">
                      Hệ thống sẽ ghi đè dữ liệu hiện tại và ghi nhật ký đầy đủ về thao tác khôi
                      phục.
                    </p>
                  </form>
                </div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                ...
                <div className="text-sm text-[color:var(--ds-text-primary)]">
                  {lastSuccess?.ts
                    ? [formatTime(lastSuccess.ts), lastSuccess.actor || "system", successExtras]
                        .filter(Boolean)
                        .join(" • ")
              ...
              <div>
                <div className="text-xs uppercase text-[color:var(--ds-text-muted)]">
                  Lần sao lưu lỗi gần nhất
                </div>

                <div className="text-sm text-[color:var(--ds-text-primary)]">
                  {lastFailure?.ts
                    ? [formatTime(lastFailure.ts), lastFailure.actor || "system", failureExtras]
                        .filter(Boolean)
                        .join(" • ")
              ...
              {filteredLogs.length === 0 ? (
                ...
                filteredLogs.map((entry, index) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 whitespace-nowrap align-top">
                      {(entry.category || inferCategoryFromAction(entry.action)).toUpperCase()}
                    </td>
