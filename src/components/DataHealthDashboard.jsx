...
export default function DataHealthDashboard({ currentUser, canManage = false }) {
  ...
  const metrics = useMemo(() => {
    ...
    const lockedCount = Array.isArray(policyOverview.lockedSources)
      ? policyOverview.lockedSources.length
      : 0;
  ...
  const lastBackupAt = backupSummary.lastSuccess?.ts
    ? formatDate(backupSummary.lastSuccess.ts)
    : "Chưa có";

  const lastBackupRelative = backupHealth.lastSuccessAt
    ? formatRelativeTime(backupHealth.lastSuccessAt)
    : "Không xác định";

  const lastBackupFile = backupSummary.lastSuccess?.meta?.file || "—";

  const lastFailureAt = backupSummary.lastFailure?.ts
    ? formatDate(backupSummary.lastFailure.ts)
    : null;
  ...
  const nextBackupRun =
    backupSchedule.active === false ? "Đang tắt" : backupSchedule.nextRunHuman || "Không xác định";
  ...
  const databaseUpdatedAt = databaseStorage.lastModifiedAt
    ? formatDate(databaseStorage.lastModifiedAt)
    : "Không rõ";

  const sqliteStats = databaseStorage.sqliteStats || {};

  const sqliteStatsAvailable =
    typeof sqliteStats.pageCount === "number" && sqliteStats.pageCount >= 0;
  ...
  const sqliteUsedPages =
    sqliteStatsAvailable && sqliteTotalPages !== null
      ? Math.max(0, sqliteTotalPages - (sqliteFreePages ?? 0))
      : null;

  const sqlitePageSizeLabel = Number.isFinite(sqliteStats.pageSizeBytes)
    ? formatBytes(sqliteStats.pageSizeBytes)
    : "—";

  const sqliteUsedLabel = Number.isFinite(sqliteStats.usedBytes)
    ? formatBytes(sqliteStats.usedBytes)
    : "—";
  ...
  const sqliteUsedPercentLabel = Number.isFinite(sqliteStats.usedPercent)
    ? formatPercent(sqliteStats.usedPercent)
    : null;

  const sqliteFreePercentLabel = Number.isFinite(sqliteStats.freePercent)
    ? formatPercent(sqliteStats.freePercent)
    : null;

  const diskUsedPercent =
    typeof diskInfo.usedPercent === "number"
      ? Math.max(0, Math.min(100, diskInfo.usedPercent))
      : null;
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-50">
              Tổng quan sức khỏe dữ liệu
            </h2>

            <p className="text-sm text-gray-500 dark:text-gray-400">
              Theo dõi chất lượng dữ liệu tờ khai, cảnh báo thiếu thông tin và trạng thái đồng bộ
              theo thời gian thực.
            </p>
          </div>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <div className="rounded border border-gray-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          <div className="flex items-center justify-between gap-2">
            <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">
              Trạng thái sao lưu CSDL
            </h3>

            <span
              className={clsx("rounded px-2 py-0.5 text-xs font-semibold", backupSeverity.badge)}
            >
              {backupSeverityLabel}
            </span>
          </div>
              ...
              {(backupSummary.recent || []).slice(0, 4).map((entry) => {
                ...
                return (
                  <div
                    ...
                    className={clsx(
                      "rounded border px-2 py-1 text-[11px] leading-relaxed",
          ...
          <dl className="mt-3 space-y-2 text-xs text-gray-600 dark:text-gray-300">
            ...
            {databaseStorage.warningCode === "memory_db" && (
              <div className="rounded border border-amber-200 bg-amber-50 p-2 text-[11px] text-amber-700 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-200">
                Hệ thống đang chạy CSDL ở chế độ bộ nhớ. Hãy cấu hình file thực tế để sao lưu được
                dữ liệu.
              </div>
            )}
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 h-2 w-full rounded-full bg-gray-200 dark:bg-slate-700">
              <div
                ...
                style={{
                  width: `${diskUsedPercent !== null ? Math.min(100, Math.max(0, diskUsedPercent)) : 0}%`,
                }}
              />
            </div>
        ...
        <div className="rounded border border-gray-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          <div className="flex items-center justify-between gap-2">
            <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">
              Trạng thái SQL Server
            </h3>

            <span
              className={clsx(
                "rounded px-2 py-0.5 text-xs font-semibold",
                (severityStyles[sqlHealthIndicator.severity] || severityStyles.info).badge,
              )}
            >
          ...
          <div className="mt-3 space-y-2 text-sm text-gray-600 dark:text-gray-300">
            ...
            {sqlHealth?.code && (
              <div className="text-[11px] text-gray-500 dark:text-gray-400">
                Mã lỗi: {sqlHealth.code}
              </div>
            )}

            {sqlHealth?.number && (
              <div className="text-[11px] text-gray-500 dark:text-gray-400">
                SQL Number: {sqlHealth.number}
              </div>
            )}
          </div>
        </div>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="mt-3 space-y-3 text-sm">
            {duplicateGroups.length === 0 ? (
              <p className="text-xs text-gray-500 dark:text-gray-400">
                Không phát hiện nhóm trùng nào.
              </p>
            ) : (
              duplicateGroups.map((group) => (
                ...
                >
                  ...
                  <div className="mt-2 text-xs text-amber-700/80 dark:text-amber-100/70">
                    Giữ lại: {group.keep?.so_tk_full || group.keep?.so_tk} • Cập nhật:{" "}
                    {formatDate(group.keep?.updatedAt)}
                  </div>
                    ...
                    <ul className="mt-2 list-disc space-y-1 pl-5 text-xs">
                      {group.duplicates.slice(0, 4).map((row, index) => (
                        <li key={`${group.key}_${row.so_tk || index}`}>
                          {row.so_tk_full || row.so_tk} • NV: {row.staff || "—"} • Tổ:{" "}
                          {row.team || "—"} • Cập nhật {formatDate(row.updatedAt)}
                        </li>
                      ))}
        ...
        <div className="rounded border border-gray-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">
              Đánh giá lần cuối:{" "}
              {summary?.alerts?.lastEvaluatedAt
                ? formatDate(summary.alerts.lastEvaluatedAt)
                : "Chưa có"}
            </span>
          </div>
              ...
              alertEntries.map((alert) => (
                ...
                >
                  ...
                  <div className="mt-1 text-xs">
                    MST: {alert.mst || "—"} • Công ty: {alert.company || "—"}
                  </div>

                  <div className="mt-1 text-xs">
                    Thiếu: {Array.isArray(alert.missing) ? alert.missing.join(", ") : "—"}
                  </div>
                  ...
                  <div className="mt-1 text-xs opacity-80">
                    Cảnh báo lần cuối: {formatDate(alert.lastAlertAt)}
                  </div>
                </div>
              ))
            )}
      ...
      <section className="rounded border border-gray-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">
              Chính sách tự động trùng 11 số
            </h3>
              ...
              <label className="space-y-1 text-xs font-medium text-gray-600 dark:text-gray-300">
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setPolicyForm((prev) => ({
                      ...(prev || {}),
                      autoNotifyAfterDays: raw === "" ? "" : Number(raw),
                    }));
                  }}
              ...
              <label className="space-y-1 text-xs font-medium text-gray-600 dark:text-gray-300">
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setPolicyForm((prev) => ({
                      ...(prev || {}),
                      notifyCooldownHours: raw === "" ? "" : Number(raw),
                    }));
                  }}
                  disabled={policyInputsDisabled}
                  readOnly={!canEditPolicy}
              ...
              <label className="space-y-1 text-xs font-medium text-gray-600 dark:text-gray-300">
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setPolicyForm((prev) => ({
                      ...(prev || {}),
                      evaluationWindowDays: raw === "" ? "" : Number(raw),
                    }));
                  }}
                  disabled={policyInputsDisabled}
                  readOnly={!canEditPolicy}
              ...
              <label className="space-y-1 text-xs font-medium text-gray-600 dark:text-gray-300">
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setPolicyForm((prev) => ({
                      ...(prev || {}),
                      autoLockAfterGroups: raw === "" ? "" : Number(raw),
                    }));
                  }}
                  disabled={policyInputsDisabled}
                  readOnly={!canEditPolicy}
              ...
              <label className="space-y-1 text-xs font-medium text-gray-600 dark:text-gray-300">
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setPolicyForm((prev) => ({
                      ...(prev || {}),
                      minGroupSizeForLock: raw === "" ? "" : Number(raw),
                    }));
                  }}
                  disabled={policyInputsDisabled}
                  readOnly={!canEditPolicy}
              ...
              <label className="space-y-1 text-xs font-medium text-gray-600 dark:text-gray-300">
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setPolicyForm((prev) => ({
                      ...(prev || {}),
                      autoUnlockAfterDays: raw === "" ? "" : Number(raw),
                    }));
                  }}
                  disabled={policyInputsDisabled}
                  readOnly={!canEditPolicy}
            ...
            <div className="rounded border border-gray-200 p-3 text-xs dark:border-slate-700 dark:bg-slate-800">
              ...
              <div className="max-h-64 overflow-auto">
                {policySourceBreakdown.length === 0 ? (
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Chưa có thống kê nguồn dữ liệu.
                  </p>
                ) : (
                      ...
                      >
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div>
                            ...
                            <div className="text-[11px] text-gray-500 dark:text-gray-400">
                              {item.awaitingActionGroups || 0} nhóm chờ xử lý •{" "}
                              {item.pendingReviewGroups || 0} nhóm chờ rà soát
                            </div>
                          </div>
                  ...
                  policyLockedSources.map((item) => (
                    ...
                    >
                      ...
                      <div className="mt-1 text-[11px]">
                        Khóa bởi: {item.lockedBy || "Hệ thống"} •{" "}
                        {item.lockedAt ? formatDate(item.lockedAt) : "Không rõ thời gian"}
                      </div>

                      {item.reason && (
                        <div className="mt-1 text-[11px] opacity-80">Lý do: {item.reason}</div>
                      )}
                    </div>
                  ))
                )}
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded border border-gray-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">
            Sự kiện SQL Server gần đây
          </h3>

          <div className="mt-3 space-y-2 text-sm">
            {sqlTimeouts.length === 0 ? (
              <p className="text-xs text-gray-500 dark:text-gray-400">
                Không ghi nhận timeout trong thời gian gần đây.
              </p>
            ) : (
              sqlTimeouts.map((event) => (
                ...
                >
                  ...
                  <div className="mt-1 text-xs">
                    {event.message || "Timeout kết nối SQL Server"}
                  </div>
        ...
        <div className="rounded border border-gray-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">
              {combinedNotifications.length} sự kiện mới
            </span>
          </div>
            ...
            {combinedNotifications.length === 0 ? (
              ...
              combinedNotifications.map((event) => (
                ...
                >
                  ...
                  {event.message && (
                    <div className="mt-1 whitespace-pre-wrap text-sm">{event.message}</div>
                  )}
                </div>
              ))
            )}
