...
export default function ExportAuditReport() {
  ...
  const { data, loading, error, execute } = useAsyncRequest(
    async ({ signal }, params) => {
      ...
      const response = await fetchWithAuth(`/api/reports/export/audit?${query.toString()}`, {
        signal,
      });
  ...
  const totalPages =
    auditData.pageCount || Math.max(1, Math.ceil((auditData.total || 0) / pageSize));
    ...
    <div className="space-y-6">
      <section className="rounded-lg border border-amber-200 bg-white p-4 shadow-sm dark:border-amber-500/40 dark:bg-slate-900">
        <header className="mb-4">
          <h2 className="text-lg font-semibold text-amber-700 dark:text-amber-300">
            Tra cứu lịch sử export
          </h2>

          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Lọc theo khoảng ngày, loại báo cáo hoặc từ khóa (tài khoản, mã xác thực, địa chỉ IP) để
            hỗ trợ tra soát việc tải file Excel có watermark.
          </p>
        </header>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              ...
              {summary.latestCreatedAt
                ? formatDateTime(summary.latestCreatedAt, { withSeconds: true })
                : "—"}
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Gần nhất:{" "}
              {latestView?.viewedAt
                ? formatDateTime(latestView.viewedAt, { withSeconds: true })
                : "—"}
            </p>

            <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              <div>
                Tài khoản: {latestView?.displayName || latestView?.username || "Chưa xác định"}
              </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        {recentAccesses.length === 0 ? (
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Chưa ghi nhận lượt truy cập nào.
          </p>
        ) : (
            ...
            {recentAccesses.map((item) => (
              <li key={`${item.id}-${item.viewedAt}`} className="py-2">
                ...
                <div className="mt-1 text-xs text-gray-600 dark:text-gray-300">
                  Tài khoản: {item.displayName || item.username || "Chưa xác định"} (
      ...
      <section className="rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-700">
          <div>
            <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100">
              Danh sách lịch sử export
            </h3>

            <p className="text-sm text-gray-500 dark:text-gray-400">
              Hiển thị {entries.length.toLocaleString("vi-VN")} /{" "}
              {auditData.total.toLocaleString("vi-VN")} lượt tải.
            </p>
          </div>

          {loading && (
            <span className="text-sm text-amber-600 dark:text-amber-300">Đang tải dữ liệu...</span>
          )}
        </div>
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {entries.length === 0 ? (
                ...
                entries.map((entry) => (
                  <tr
                    key={`${entry.id}-${entry.requestId || entry.signature}`}
                    className="bg-white odd:bg-slate-50 dark:bg-slate-900 dark:odd:bg-slate-800/70"
                  >
                    ...
                    <td className="align-top px-4 py-3">
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400">
                        {entry.filename || "Không rõ tên file"}
                      </div>
                    </td>

                    <td className="align-top px-4 py-3">
                      <div className="font-medium text-gray-800 dark:text-gray-100">
                        {entry.displayName || entry.username || "Không rõ"}
                      </div>
                    ...
                    <td className="align-top px-4 py-3">
                      <div className="font-mono text-sm text-amber-600 dark:text-amber-300">
