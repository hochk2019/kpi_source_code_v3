...
function resolveCategoryDefaults(category, settings) {
  ...
  if (config.type === "grade") {
    ...
    const defaultGrade = allowed.find((item) => Number(item.value) === overrideValue) ||
  ...
  if (config.type === "hybrid") {
    ...
    let mode = normalizeStr(
      overrides.defaultMode || config.defaultMode || allowedModes[0] || "",
    ).toLowerCase();
...
export default function KPIAdjustments({ currentUser }) {
  ...
  const businessDirectory = businessData.directory || {
    entries: [],
    byMst: new Map(),
    byCompany: new Map(),
  };
  ...
  const quickDeclarationSuggestions = useMemo(
    () => declarationSuggestions.slice(0, 5),
    [declarationSuggestions],
  );
  ...
  const filteredDeclarationResults = useMemo(() => {
    ...
    if (digits && primaryResults.length < 10) {
      ...
      for (const row of rows) {
        ...
        const hasMatch =
          (soTkDigits && soTkDigits.includes(digits)) || (mstDigits && mstDigits.includes(digits));
    ...
    (draft, info = {}) => {
      ...
      if (incomingMst) {
        ...
        if (mstEntry?.company) {
          ...
          if (
            !next.companyName ||
            normalizeName(next.companyName) !== normalizeName(incomingCompany)
          ) {
      ...
      if (incomingCompany) {
        ...
        if (companyEntry) {
          ...
          const hasCurrent =
            currentMst && companyEntry.msts instanceof Set && companyEntry.msts.has(currentMst);

          if (!hasCurrent) {
            const firstMst =
              companyEntry.msts instanceof Set ? Array.from(companyEntry.msts)[0] : undefined;
  ...
  const handleModeChange = (nextMode) => {
    setForm((prev) => {
      ...
      const availableModes = Array.isArray(config.modes)
        ? config.modes.map((item) => item.value)
        : [];
  ...
  const handleSettingsSubmit = (event) => {
    ...
    for (const [category, draftEntry] of Object.entries(settingsDraft)) {
      ...
      if (
        ...
        Object.prototype.hasOwnProperty.call(draftEntry, "extraUnitPoints")
      ) {
  ...
  const handleSubmit = (event) => {
    ...
    if (categoryConfig.requiresLicenseCode) {
      ...
      if (normalizedCode) {
        ...
        Array.isArray(categoryConfig.licenseOptions) &&
        categoryConfig.licenseOptions.length
      ) {
        payload.licenseCode =
  ...
  const licenseOptions = Array.isArray(formCategoryConfig.licenseOptions)
  ...
  const detailCategoryConfig = detailData
    ? KPI_ADJUSTMENT_CATEGORY_CONFIG[detailData.category] || {}
    : {};
  ...
  const detailExtraUnit = detailCategoryConfig.extraPointConfig
    ? Number.parseFloat(
        detailData?.extraUnitPoints ?? detailCategoryConfig.extraPointConfig?.defaultUnit ?? 0,
      ) || 0
    : 0;
        ...
        <CardContent className="grid gap-4 pt-0 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-lg border border-border bg-muted/50 p-4 text-sm shadow-sm">
            ...
            <div className="mt-1 text-2xl font-semibold text-blue-600">
              {formatDecimal(stats.totalPoints)}
            </div>
          </div>
        </CardContent>
      </Card>
            ...
            <div className="space-y-4 text-sm text-foreground">
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <div className="mt-1 font-medium text-foreground">
                    {KPI_ADJUSTMENT_CATEGORY_CONFIG[detailData.category]?.label ||
                      detailData.category}
                  </div>
                </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <div className="mt-1 font-medium text-foreground">
                    {formatDecimal(detailData.quantity ?? 0)}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-1 font-medium text-foreground">
                    {formatDecimal(detailData.unitPoints ?? 0)}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-1 font-semibold text-foreground">
                    {formatDecimal(detailData.totalPoints ?? 0)}
                  </div>
                </div>
                ...
                {detailCategoryConfig.extraPointConfig ? (
                  <>
                    <div>
                      ...
                      <div className="mt-1 font-medium text-foreground">
                        {formatDecimal(detailExtraQuantity)}
                      </div>
                    </div>

                    <div>
                      <div className="text-xs font-medium uppercase text-muted-foreground">
                        {detailCategoryConfig.extraPointConfig.unitLabel ||
                          "Điểm bổ sung mỗi đơn vị"}
                      </div>

                      <div className="mt-1 font-medium text-foreground">
                        {formatDecimal(detailExtraUnit)}
                      </div>
                    </div>
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <div className="mt-1 font-medium text-foreground">
                        {formatDecimal(detailExtraTotal)}
                      </div>
                    </div>
                  </>
                ) : null}
              ...
              <div>
                ...
                <div className="mt-1 whitespace-pre-line text-foreground">
                  {detailData.note || "—"}
                </div>
              </div>
              ...
              {detailIntent === "reject" ? (
                <div className="space-y-2">
                  <label
                    ...
                    htmlFor={FORM_FIELD_IDS.decisionNote}
                  >
      ...
      <Dialog open={guidanceOpen} onOpenChange={setGuidanceOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Tham khảo điểm mặc định, điểm bổ sung và cách tính cho từng nhóm hạng mục theo cấu
              hình hiện tại.
            </DialogDescription>
          </DialogHeader>
                                  ...
                                  <div className="text-xs text-muted-foreground">
                                    ...
                                    <ul className="mt-1 space-y-1">
                                      {item.gradeOptions.map((grade) => (
                                        <li key={`${item.key}-grade-${grade.value}`}>
                                          {grade.label}
                                        </li>
                                      ))}
                                    </ul>
                    ...
                    <div className="mt-3 space-y-3">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateSettingsDraft(category, "defaultUnit", e.target.value)
                          }
                          className="mt-1"
                        />
                      </div>
                      ...
                      {config.extraPointConfig ? (
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              updateSettingsDraft(category, "extraUnitPoints", e.target.value)
                            }
                            className="mt-1"
                          />
                        </div>
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                updateSettingsDraft(category, "defaultMode", e.target.value)
                              }
                            >
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="grid gap-4 text-sm md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    const matched = staffOptions.find(
                      (option) => normalizeStr(option.name) === normalizedName,
                    );
              ...
              <div>
                <label
                  className="text-sm font-medium text-foreground"
                  htmlFor={FORM_FIELD_IDS.taxCode}
                >
                  Mã số thuế
                </label>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Chọn MST để tự điền tên công ty tương ứng.
                </p>
              </div>
              ...
              <div>
                <label
                  className="text-sm font-medium text-foreground"
                  htmlFor={FORM_FIELD_IDS.category}
                >
                  Hạng mục
                </label>
              ...
              {formCategoryConfig.requiresLicenseCode ? (
                <div>
                  <label
                    className="text-sm font-medium text-foreground"
                    htmlFor={FORM_FIELD_IDS.license}
                  >
                    Mã giấy phép
                  </label>
            ...
            <div className="grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
              <div className="space-y-4">
                ...
                <div>
                  <label
                    className="text-sm font-medium text-foreground"
                    htmlFor={FORM_FIELD_IDS.references}
                  >
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setForm((prev) => ({ ...prev, referencesInput: e.target.value }))
                    }
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      {!isHybridFixed ? (
                        <div>
                          <label
                            className="text-sm font-medium text-foreground"
                            htmlFor={FORM_FIELD_IDS.quantity}
                          >
                            Số lượng
                          </label>
                      ...
                      <div>
                        ...
                        >
                          {formCategoryConfig.type === "hybrid" && isHybridFixed
                            ? "Điểm cố định"
                            : "Điểm mỗi đơn vị"}
                        </label>
                    ...
                    {formCategoryConfig.extraPointConfig ? (
                      <div className="grid gap-3 md:grid-cols-2">
                        <div>
                          <label
                            className="text-sm font-medium text-foreground"
                            htmlFor={FORM_FIELD_IDS.extraQuantity}
                          >
                            {formCategoryConfig.extraPointConfig.quantityLabel ||
                              "Số lượng bổ sung"}
                          </label>
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setForm((prev) => ({ ...prev, extraQuantity: e.target.value }))
                            }
                            className="mt-1"
                          />
                        </div>
                        ...
                        <div>
                          <label
                            className="text-sm font-medium text-foreground"
                            htmlFor={FORM_FIELD_IDS.extraUnit}
                          >
                            {formCategoryConfig.extraPointConfig.unitLabel ||
                              "Điểm bổ sung mỗi đơn vị"}
                          </label>
        ...
        <CardContent>
          <div className="grid gap-4 text-sm md:grid-cols-3">
            <div>
              <label
                className="text-sm font-medium text-foreground"
                htmlFor={FORM_FIELD_IDS.filterMonth}
              >
                Lọc theo tháng
              </label>
            ...
            <div>
              <label
                className="text-sm font-medium text-foreground"
                htmlFor={FORM_FIELD_IDS.filterStatus}
              >
                Trạng thái
              </label>
              ...
              <tbody className="divide-y divide-border/60 text-foreground">
                {filteredAdjustments.length ? (
                  filteredAdjustments.map((item) => {
                    const label =
                      KPI_ADJUSTMENT_CATEGORY_CONFIG[item.category]?.label || item.category;
                    ...
                    const extraQuantity = rowConfig.extraPointConfig
                      ? Number.parseFloat(item.extraQuantity ?? 0) || 0
                      : 0;

                    const extraUnitPoints = rowConfig.extraPointConfig
                      ? Number.parseFloat(
                          item.extraUnitPoints ?? rowConfig.extraPointConfig.defaultUnit ?? 0,
                        ) || 0
                      : 0;
                            ...
                            <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                              ...
                              {rowConfig.extraPointConfig && extraQuantity > 0 ? (
                                <Badge variant="outline">
                                  Bổ sung: {formatDecimal(extraQuantity)} x{" "}
                                  {formatDecimal(extraUnitPoints)} ({formatDecimal(extraTotal)})
                                </Badge>
                              ) : null}
                            </div>
                          </div>
                        ...
                        <td className="px-4 py-2 align-top">
                          <div className="flex flex-wrap gap-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              type="button"
                              onClick={() => setDetailEntry({ entry: item, intent: "view" })}
                            >
                              Chi tiết
                            </Button>
                              ...
                              <>
                                {item.status !== "approved" ? (
                                  <Button
                                    ...
                                    type="button"
                                    onClick={() =>
                                      setDetailEntry({ entry: item, intent: "approve" })
                                    }
                                  >
                                    Duyệt
                                  </Button>
                                ...
                                {item.status !== "rejected" ? (
                                  <Button
                                    ...
                                    type="button"
                                    onClick={() =>
                                      setDetailEntry({ entry: item, intent: "reject" })
                                    }
                                  >
                                    Từ chối
                                  </Button>
