...
function CompanySummaryTable({
  ...
  const activeColumns = columns.filter((col) =>
    col.visibleKey ? visibleColumns[col.visibleKey] !== false : true,
  );
...
function TopStaffWidget({
  ...
  const effectiveVisibleCount =
    resolvedPreference === "auto" ? autoVisibleCount : resolvedPreference;

  const visibleCount = Math.max(
    1,
    Number.isFinite(effectiveVisibleCount) ? effectiveVisibleCount : autoVisibleCount || 5,
  );

  const displayedKpiData = useMemo(
    () => kpiData.slice(0, Math.max(visibleCount, 1)),
    [kpiData, visibleCount],
  );
  ...
  const maxKPI = hasKpiData
    ? Math.max(...displayedKpiData.map((item) => item.stats.kpi || 0), 1)
    : 1;
...
function TrendLineChart({ data, comparison, palette = DEFAULT_CHART_COLORS }) {
  ...
  const deltaClass =
    deltaKPI > 0 ? "text-emerald-600" : deltaKPI < 0 ? "text-red-600" : "text-gray-600";
                  ...
                  <table className="min-w-full text-sm">
                    ...
                    <tbody>
                      {chunkRows.map((row, idx) => {
                        const licenseCodes = Array.isArray(row.licenseCodes)
                          ? row.licenseCodes
                          : [];
      ...
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Thành viên:{" "}
            {memberNames.length ? memberNames.join(", ") : "Chưa có thành viên trong roster"}
          </p>
        ...
        <>
          <div className="overflow-auto rounded border">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {sortStatsCollection(members, memberSortKey, (item) => item.name || "").map(
                  (member, idx) => (
                    <tr
                      key={member.key || idx}
                      className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}
                    >
                      ...
                      {showLicenses ? (
                        <td className="px-3 py-1.5 text-right">
                          {formatInt(member.stats.licenses)}
                        </td>
                      ) : null}

                      {showCo ? (
                        <td className="px-3 py-1.5 text-right">
                          {formatInt(member.stats.co ?? 0)}
                        </td>
                      ) : null}

                      {showCoLines ? (
                        <td className="px-3 py-1.5 text-right">
                          {formatInt(member.stats.coLines ?? 0)}
                        </td>
                      ) : null}
                          ...
                          {chunkRows.map((row, idx) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {showItems ? (
                                  <td className="px-3 py-1.5 text-right">
                                    {formatInt(row.num_items)}
                                  </td>
                                ) : null}

                                {showLicenses ? (
                                  <td className="px-3 py-1.5 text-right">
                                    {formatInt(row.licenses)}
                                  </td>
                                ) : null}
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setDetailPage((prev) => Math.min(prev + 1, totalDetailPages - 1))
                      }
                      disabled={isLastDetailPage}
                      className={`inline-flex items-center justify-center rounded-full border px-2 py-1 font-semibold transition-colors ${
                        isLastDetailPage
...
export default function ReportViewer({ canExport = true, currentUser = null }) {
  ...
  }, [selectedStaff, staffViewMode, filteredStaffList.length, detailPageSize, staffDetailPage]);
  ...
  }, [selectedTeam, teamViewMode, filteredTeamList.length, detailPageSize, teamDetailPage]);
  ...
  const handleSaveSchedule = (event) => {
    ...
    if (!String(payload.recipients || "").trim()) {
      toast.warning?.(
        "Nhập danh sách email nhận báo cáo (ngăn cách bởi dấu phẩy hoặc xuống dòng).",
      );
  ...
  const handleDeleteSchedule = (schedule) => {
    ...
    const confirmed = window.confirm(`Xoá lịch gửi "${schedule.name || "Báo cáo KPI"}"?`);
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
          ...
          <div className="rounded-xl border border-[color:var(--ds-border-subtle)] bg-[color:var(--ds-surface-card)] p-3 text-sm text-[color:var(--ds-text-secondary)]">
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs uppercase text-[color:var(--ds-text-muted)]">
                Thông tin báo cáo
              </span>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-[color:var(--ds-text-secondary)]">
                Thiết lập gửi tự động file Excel/PDF theo tuần hoặc tháng tới danh sách email mong
                muốn.
              </p>
            </div>
              ...
              >
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      handleScheduleFieldChange("recipientsInput", event.target.value)
                    }
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {scheduleDraft.frequency === "weekly" ? (
                    <select
                      ...
                      onChange={(event) =>
                        handleScheduleFieldChange("dayOfWeek", Number(event.target.value))
                      }
                      className="rounded border border-[color:var(--ds-border-subtle)] bg-white px-3 py-2 text-sm text-[color:var(--ds-text-primary)] shadow-sm focus:border-[color:var(--ds-border-strong)] focus:outline-none"
                    >
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(event) =>
                          handleScheduleFieldChange("dayOfMonth", Number(event.target.value))
                        }
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <label className="mt-1 inline-flex items-center gap-2 text-xs text-[color:var(--ds-text-secondary)]">
                    <input
                      ...
                      onChange={(event) =>
                        handleScheduleFieldChange("active", event.target.checked)
                      }
                    />
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              <div className="lg:col-span-2">
                        ...
                        paginatedAppliedAdjustments.map((item) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="px-3 py-2">
                                {item.adjustment?.label || item.loai_hinh}
                              </td>
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-xs text-[color:var(--ds-text-secondary)]">
                  ...
                  <div className="flex items-center gap-2">
                    <span>
                      Trang {totalAdjustmentPages ? currentAdjustmentPage + 1 : 0}/
              ...
              <div className="space-y-6">
                <section className="space-y-3 rounded-xl border border-[color:var(--ds-border-subtle)] bg-[color:var(--ds-surface-muted)] p-4">
                  ...
                  <div className="text-xs text-[color:var(--ds-text-secondary)]">
                    Từ{" "}
                    {formatInt(
                      adjustmentsReport.approvedCount || adjustmentsReport.appliedCount || 0,
                    )}{" "}
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="mt-1 text-xs text-[color:var(--ds-text-muted)]">
                              Số lượt:{" "}
                              <span className="font-semibold text-[color:var(--ds-text-primary)]">
                                {formatOptionalInt(item.quantity)}
                              </span>
                            </div>
                          </li>
                        );
