...
function TierEditor({ tiers = [], onChange, disabled = false, title = "Bậc cộng thêm" }) {
  ...
  const handleChange = (index, key, value) => {
    onChange(safeTiers.map((tier, idx) => (idx === index ? { ...tier, [key]: value } : tier)));
  };
      ...
      <datalist id={listId}>
        {normalizedOptions.map((item) => (
          <option
            ...
            value={item.value}
            label={
              item.count ? `${item.value} (${item.count.toLocaleString("vi-VN")})` : item.value
            }
          />
        ))}
      </datalist>
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-4">
      {isReadOnly && (
        <div className="rounded border border-amber-300 bg-amber-50 p-3 text-sm text-amber-700">
          Bạn đang xem quy tắc KPI ở chế độ chỉ xem. Các trường cấu hình bị khóa; vẫn có thể dùng
          khu vực test để kiểm tra điểm KPI.
        </div>
      )}
        ...
        <CardContent className="space-y-8">
          ...
          <div className="space-y-3 rounded border p-3">
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
              <span>
                Phiên bản đang chỉnh:{" "}
                <strong>{currentVersion !== null ? currentVersion : "—"}</strong>
              </span>
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  <div>
                    <label className="text-sm text-gray-600">
                      Điểm mặc định mỗi loại giấy phép
                    </label>
          ...
          <div className="space-y-3 rounded border p-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Ước tính tổng điểm dựa trên {data.length.toLocaleString("vi-VN")} tờ khai đang lưu
              ...
              <Button
                ...
                disabled={simRunning || !data.length}
                variant="outline"
              >
            ...
            {simResult ? (
              ...
              <p className="text-xs text-gray-500">
                Chưa có dữ liệu mô phỏng. Nhấn "Chạy mô phỏng" để xem kết quả.
              </p>
            )}
          </div>
          ...
          <div className="space-y-3 rounded border p-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="outline" onClick={exportCurrentRule}>
                Xuất bộ đang mở
              </Button>
              ...
              <Button variant="outline" onClick={exportAllRules}>
          ...
          <div className="space-y-2 rounded border border-[color:var(--ds-border-subtle)] bg-[color:var(--ds-surface-card)] p-3">
            ...
            {historyCollapsed ? (
              <p className="text-xs text-[color:var(--ds-text-muted)]">
                Đã thu gọn lịch sử. Nhấn “Mở rộng” để xem chi tiết.
              </p>
            ) : historyEntries.length === 0 ? (
              <p className="text-xs text-[color:var(--ds-text-muted)]">
                Chưa có ghi nhận lịch sử nào.
              </p>
            ) : (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {historyEntries.map((entry, index) => {
                      const rowId =
                        entry.id || entry.snapshot?.id || `${entry.updatedAt || ""}-${index}`;
                      ...
                      return (
                        <React.Fragment key={rowId}>
                          <tr className="border-t border-gray-100">
                            ...
                            <td className="px-2 py-2 text-xs text-gray-600">
                              {entry.applyFrom || "Áp dụng ngay"}
                            </td>

                            <td className="px-2 py-2 text-sm text-gray-700">
                              {entry.name || entry.snapshot?.name || rowId}
                            </td>
