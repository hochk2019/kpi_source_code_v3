...
function TeamManager({ canEdit = true, currentUser = null }) {
  ...
  const refreshHistory = useCallback(() => {
    setTeamHistory(getAuditLogs(100).filter((entry) => entry?.action?.startsWith("team")));
  ...
  const handleSave = () => {
    ...
    try {
      ...
      alert(
        changed ? "Đã lưu tổ đội và đồng bộ dữ liệu MST thành công." : "Đã lưu tổ đội thành công.",
      );
    } catch (error) {
      console.error(error);
    ...
    <div className="p-6 space-y-6">
      {isReadOnly && (
        <div className="rounded border border-amber-300 bg-amber-50 p-3 text-sm text-amber-700">
          Bạn đang xem quản lý tổ đội ở chế độ chỉ xem. Đăng nhập bằng tài khoản quản trị hoặc được
          phân quyền để thêm, sửa hoặc điều chuyển thành viên.
        </div>
      )}
      ...
      <p className="text-sm text-gray-600">
        ...
        thành viên, doanh nghiệp phụ trách và điều chỉnh.
      </p>
