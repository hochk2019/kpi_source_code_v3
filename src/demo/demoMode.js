...
function buildBootstrapStore() {
  ...
  const licenseSets = [["ZB02"], ["ZB03"], ["ZB02", "ZB03"], ["ZC01"]];
  ...
  const bootstrapStore = {
    ...
    decl_alert_config_v1: JSON.stringify({
      enabled: true,
      thresholdDays: 2,
    ...
    kpi_users_v1: JSON.stringify([
      { username: "admin", role: "admin", name: "Quản trị viên", permissions: {} },
    ]),
    ...
    duplicate_policy_state_v1: JSON.stringify({
  ...
  window.fetch = async (input, init = {}) => {
    ...
    try {
      ...
      if (url.includes("/api/feedback") && method === "POST")
        return createResponse(responses.feedbackSubmit);
  ...
  class DemoEventSource {
    ...
    addEventListener(type, listener) {
      if (type === "message" && typeof listener === "function") {
        setTimeout(
          () => listener({ data: JSON.stringify(responses.notifications.events[1]) }),
          1500,
        );
      }
    }
