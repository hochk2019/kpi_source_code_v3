...
export interface TabDefinition {
  ...
  isVisible?: (access: AppShellAccess) => boolean;
}

export interface AppShellAccess {
...
export interface NavSection extends NavSectionDefinition {
  tabs: TabDefinition[];
}
...
export const APP_TAB_DEFINITIONS = Object.freeze([
  ...
  },
  {
    ...
    label: 'Gán MST & HQ',
    tooltip: 'Quản lý gán MST và đại lý hải quan',
    commandLabel: 'Đi tới tab Gán MST & HQ',
    commandDescription: 'Quản lý gán MST cho doanh nghiệp và đại lý hải quan',
    commandKeywords: ['mst', 'hq', 'gan mst', 'dai ly', 'hai quan'],
  },
  {
    id: 'import',
    sectionId: 'operations',
...
export function getAppShellAccess(currentUser: AuthAccountView | null | undefined): AppShellAccess {
...
export function getVisibleAppTabs(currentUser: AuthAccountView | null | undefined): TabDefinition[] {
...
export function getVisibleAppTabIds(currentUser: AuthAccountView | null | undefined): Set<string> {
  return new Set(getVisibleAppTabs(currentUser).map((tab) => tab.id));
}

export function getAppTabDefinition(tabId: string): TabDefinition | null {
  return APP_TAB_DEFINITION_MAP.get(tabId) || null;
}

export function getAppNavigationSectionDefinition(sectionId: string): NavSectionDefinition | null {
  return APP_SECTION_DEFINITION_MAP.get(sectionId) || null;
}

export function getVisibleAppNavigationSections(currentUser: AuthAccountView | null | undefined): NavSection[] {
  const visibleTabs = getVisibleAppTabs(currentUser);
  const sections: NavSection[] = APP_NAVIGATION_SECTION_DEFINITIONS.map((section) => ({
    ...section,
    tabs: [],
  }));
...
function createSearchParams(search: string | URLSearchParams | Record<string, string> | undefined): URLSearchParams {
...
export function resolveVisibleAppTab(
  ...
  fallbackTab: string = APP_SHELL_FALLBACK_TAB,
...
interface SerializeAppShellLocationOptions {
...
}: SerializeAppShellLocationOptions = {}): string {
  const params = createSearchParams(search);
