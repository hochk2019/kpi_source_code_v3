...
function normalizeGroup(rawGroup, fallbackGroup) {
  ...
  const next = {
    ...
    perItem: Number.isFinite(Number(input.perItem))
      ? Number(input.perItem)
      : Number(base.perItem) || 0,

    tierMode:
      normText(input.tierMode) ||
      normText(base.tierMode) ||
      (Number(base.perItem) ? "per_item" : "highest"),
...
function normalizeBonuses(rawBonuses, fallbackBonuses) {
  ...
  return {
    co: {
      ...
      points: Number.isFinite(Number(rawCo.points))
        ? Number(rawCo.points)
        : Number(base.co?.points) || 0,
...
function persistCollection(collection) {
  ...
  const activeRule =
    normalized.sets.find((rule) => rule.id === normalized.activeId) || normalized.sets[0] || null;
...
export function saveRules(ruleInput, opts = {}) {
  ...
  const incomingVersion = Number.isFinite(Number(normalizedRule.version))
    ? Number(normalizedRule.version)
    : previousVersion;
  ...
  const recalcFrom =
    opts.recalcFrom && persisted.activeId === normalizedRule.id ? normText(opts.recalcFrom) : "";
...
export function restoreRuleVersion(snapshotInput, opts = {}) {
  ...
  const restored = saveRules(
...

    {
      ...
      detail:
        opts.detail ||
        `Khôi phục phiên bản ${normalizedSnapshot.version || ""} của ${normalizedSnapshot.name}`,
    },
  );
...
function buildLicenseConfigMaps(licenseConfig) {
  ...
  for (const entry of Array.isArray(licenseConfig?.codePoints) ? licenseConfig.codePoints : []) {
    ...
    const points = Number.isFinite(Number(entry?.points))
      ? Number(entry.points)
      : Number(licenseConfig?.defaultPoints) || 0;
  ...
  for (const entry of Array.isArray(licenseConfig?.exclude?.agencies)
    ? licenseConfig.exclude.agencies
    : []) {
    const agencyKey = norm(entry?.agency);
