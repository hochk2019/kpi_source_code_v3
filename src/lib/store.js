...
const IMPORT_CONFIG_COLUMN_IDS = Object.freeze([...IMPORT_COLUMN_IDS, ...IMPORT_AUX_COLUMN_IDS]);
...
function sanitizeMSTRow(rowInput) {
  ...
  const companyRaw = pickFirstValue(
    row,
    ["company", "company_name", "companyName", "tenCongTy"],
    row?.company,
  );
...
export function upsertMSTRows(rows, { actor = "system", detail = "" } = {}) {
  ...
  const sanitized = Array.isArray(rows) ? rows.map(sanitizeMSTRow).filter(Boolean) : [];
...
function sanitizeDeclHistoryStore(rawStore) {
  ...
  const sourceRows =
    rawStore.rows && typeof rawStore.rows === "object" && !Array.isArray(rawStore.rows)
      ? rawStore.rows
      : {};
...
function sanitizeRoster(data) {
  ...
  const teamsInput = Array.isArray(data.teams) ? data.teams : Array.isArray(data) ? data : [];
...
export function setTeamRoster(next, { actor = "system", detail = "" } = {}) {
  const normalizedInput =
    Array.isArray(next?.teams) || Array.isArray(next) ? next : deepCloneRoster(DEFAULT_ROSTER);

  const sanitized = sanitizeRoster(
    Array.isArray(normalizedInput) ? { version: 1, teams: normalizedInput } : normalizedInput,
  );
...
export function applyTeamRosterToMST(rosterLike, rows, options = {}) {
  ...
  const previousRoster = options?.previousRoster ? sanitizeRoster(options.previousRoster) : null;
  ...
  const sanitizedRows = Array.isArray(rows) ? rows.map(sanitizeMSTRow).filter(Boolean) : [];
...
function persistAndAnnotateDeclRows(rows) {
  ...
  const changed =
    annotated.length !== normalized.length || annotated.some((row, idx) => row !== normalized[idx]);
...
export function previewDeclRows(newRows, { overwrite = false, actor = "system" } = {}) {
  ...
  if (overwrite) {
    ...
    const mstSummary = ensureMSTEntriesForDeclRows(normalizedIncoming, {
      actor: actorName,
      dryRun: true,
    }) || {
      additions: [],
...
export function saveDeclRows(newRows, { overwrite = false, actor = "system", detail = "" } = {}) {
  ...
  if (overwrite) {
    ...
    return {
      ...
      insertedDeclarations: normalizedIncoming
        .map((row) => buildImportLogEntry(row))
        .filter(Boolean),
...
export function unmarkDeclRowsReviewed(
  keys,
  { actor = "system", note = "Bỏ đánh dấu rà soát" } = {},
) {
  const list = Array.isArray(keys)
    ? keys.map((key) => String(key || "").trim()).filter(Boolean)
    : [];
...
export function saveKpiAdjustmentSettings(patch, { actor = "system", permissions = {} } = {}) {
  ...
  const patchCategories =
    patch && typeof patch === "object" && typeof patch.categories === "object"
      ? patch.categories
      : null;
    ...
    for (const [rawKey, rawValue] of Object.entries(patchCategories)) {
      ...
      if (Object.prototype.hasOwnProperty.call(value, "defaultMode")) {
        ...
        const allowedModes = new Set(
          (KPI_ADJUSTMENT_CATEGORY_CONFIG[key].modes || []).map((item) => item.value),
        );
...
function clampHistory(list) {
  const entries = Array.isArray(list)
    ? list.map(normalizeAdjustmentHistoryEntry).filter(Boolean)
    : [];

  return entries.slice(-KPI_ADJUSTMENT_HISTORY_LIMIT);
}

function computeAdjustmentTotal({
  category,
  unitPoints,
  quantity,
  mode,
  ...
  if (config.type === "fixed" || config.type === "grade") {
    ...
    const targetMode =
      (config.modes || []).find((item) => item.value === normalizedMode) || config.modes?.[0];
  ...
  if (extraConfig) {
    ...
    const resolvedExtraUnit = Number.isFinite(extraUnitPoints)
      ? extraUnitPoints
      : fallbackExtraUnit;
...
function normalizeAdjustmentInput(input, { now, actor, current, permissions = {} } = {}) {
  ...
  const allowManualOverride =
    permissions?.adjustOverridePoints === true || category === "support_misc";
  ...
  const companyName = normalizeStr(
    input.companyName ?? input.company ?? current?.companyName ?? "",
  );
  ...
  const unitSource = allowManualOverride
    ? (input.unitPoints ?? input.basePoint ?? input.pointsPerUnit)
    : undefined;
  ...
  if (!Number.isFinite(unitPoints)) {
    if (!allowManualOverride && Number.isFinite(preservedUnit)) {
      ...
      const overrideModeUnits =
        override?.modeUnits && typeof override.modeUnits === "object" ? override.modeUnits : {};
      ...
      licenseCode &&
      mergedLicensePoints[licenseCode] != null
    ) {
  ...
  let quantity = Number.parseFloat(
    input.quantity ?? input.count ?? input.total ?? current?.quantity ?? 0,
  );
  ...
  if (extraConfig) {
    ...
    const extraUnitSource = allowManualOverride
      ? (input.extraUnitPoints ??
        input.bonusUnitPoints ??
        current?.extraUnitPoints ??
        current?.bonusUnitPoints)
      : undefined;
    ...
    const extraQuantitySource =
      input.extraQuantity ??
      input.bonusQuantity ??
      current?.extraQuantity ??
      current?.bonusQuantity ??
      0;
  ...
  const references = normalizeAdjustmentReferences(
    input.references ?? input.reference ?? current?.references ?? [],
  );
  ...
  const totalOverride = Number.parseFloat(
    input.totalPoints ?? input.pointsTotal ?? input.total ?? NaN,
  );
  ...
  const payload = {
    id:
      current?.id ||
      input.id ||
      `adj-${month.replace(/-/g, "")}-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
...
export function updateKpiAdjustmentStatus(
  id,
  status,
...
export function getData() {
  // RulesEditor.jsx Ä‘ang import

  return getDeclRows();
}

export function setData(rows, opts) {
  // rules.js/RulesEditor.jsx cÃ³ thá»ƒ gá»i
...
function normalizeReportScheduleEntry(entry, fallback = null) {
  ...
  const id = String(
    raw.id || base.id || `schedule-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );

  const name =
    String(raw.name ?? base.name ?? "Báo cáo KPI")
      .trim()
  ...
  const normalized = {
    ...
    dayOfMonth:
      frequency === "monthly" ? clampMonthDay(raw.dayOfMonth ?? base.dayOfMonth ?? 1) : null,
