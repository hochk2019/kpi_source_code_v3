...
function prepareFilters(rawFilters = {}) {
  ...
  const normalized = {
    ...
    noStaff: parseBooleanInput(rawFilters.noStaff) || parseBooleanInput(rawFilters.filterNoStaff),

    noTeam: parseBooleanInput(rawFilters.noTeam) || parseBooleanInput(rawFilters.filterNoTeam),

    duplicate:
      parseBooleanInput(rawFilters.duplicate) || parseBooleanInput(rawFilters.filterDuplicate11),
...
export function filterDeclRows(rows, rawFilters = {}, context = {}) {
  ...
  for (const row of list) {
    ...
    if (filters.queryLower) {
      ...
      const companyRaw = (row.cong_ty ?? row.company ?? row.ten_cong_ty ?? row.doanh_nghiep ?? "")
        .toString()
        .toLowerCase();
